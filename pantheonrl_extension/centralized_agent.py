...
if __name__ == "__main__":
    ...
    if envs.action_space.shape == tuple():
        actions = torch.zeros((n_players, args.num_steps, args.num_envs, 1)).to(device)
    else:
        actions = torch.zeros((n_players, args.num_steps, args.num_envs) + envs.action_space.shape).to(device)
    action_masks = torch.zeros((n_players, args.num_steps, args.num_envs, envs.action_space.n)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            for p in range(n_players):
                obs[p, step] = next_obs[p].obs
                action_masks[p, step] = next_obs[p].action_mask
                active[p, step] = next_obs[p].active
            ...
            with torch.no_grad():
                ...
                for p in range(n_players):
                    ...
                    values[p, step] = value.flatten()
                    actions[p, step] = action_i.unsqueeze(-1)
                    logprobs[p, step] = logprob
