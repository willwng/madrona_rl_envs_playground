...
from oldercooked_ai_py.utils import pos_distance, load_from_json
...
class OvercookedState(object):
    ...
    def all_objects_by_type(self):
        ...
        other_objs = self.player_objects_by_type
        for k in all_objs_by_type:
            if k in other_objs:
                all_objs_by_type[k].extend(other_objs[k])

        for k in other_objs:
            if k not in all_objs_by_type:
                all_objs_by_type[k] = other_objs[k]
        # all_objs_by_type.update()
